...
with left:
    st.markdown("### CAPM")
    rf = st.slider("Risk-Free Rate Rf (%) — 10yr Treasury, ~4.3% in 2025", 0.0, 8.0, 4.30, 0.05) / 100
    erp = st.slider("Equity Risk Premium ERP (%) — historical US avg ~5.5%", 3.0, 9.0, 5.50, 0.05) / 100
    beta = st.number_input(f"Beta β (live: {data['beta']:.2f})", 0.01, 5.0, float(data["beta"]), 0.01)
...
with right:
    ...
    default_eq_weight = 80 if st.session_state.ticker == "NVDA" else 85
    eq_weight = st.slider("Equity Weight E/V (%)", 0, 100, default_eq_weight, 1) / 100
    st.write(f"⇒ Debt Weight D/V = {(1-eq_weight)*100:.0f}%")
    rd = st.slider("Pre-Tax Cost of Debt Rd (%)", 0.0, 15.0, 4.00, 0.05) / 100
    tax = st.slider("Effective Tax Rate (%)", 0, 40, 21, 1) / 100
...
with sl1:
    ...
    forecast_years = st.slider("Forecast years", 3, 10, 5, 1)
    custom_rates = st.toggle("Custom rate per year (override per-year)", value=False)
    if custom_rates:
        rates=[]
        for i in range(forecast_years):
            rates.append(st.slider(f"Year {i+1} growth (%)", -50.0, 100.0, near_default if i < 3 else mid_default, 0.5)/100)
        g1 = rates[0]
        g2 = rates[-1]
    else:
        g1 = st.slider(f"Near-term growth Yrs 1–3 (%) — {st.session_state.ticker} hist avg: {near_default:.1f}%", -50.0, 100.0, near_default, 0.5) / 100
        g2 = st.slider("Mid-term growth Yrs 4+ (%) — suggested", -50.0, 100.0, mid_default, 0.5) / 100
        rates = None
    st.markdown("### Base FCF")
    base_fcf = st.number_input(f"Base FCF ($M) — auto-pulled: {data['base_fcf_m']:.1f}M", value=float(data["base_fcf_m"]), step=100.0)
with sl2:
    st.markdown("### Stage 2 — Terminal")
    terminal_g = st.slider("Perpetuity Growth Rate (%)", 0.0, 5.0, 2.50, 0.05) / 100
...
with bear:
    st.markdown("### 🐻 Bear")
    bear_g = st.slider("Growth (%)", -10, 32, 0, 1) / 100
    bear_wacc = st.slider("WACC (%)", 5, 20, 10, 1) / 100
    bear_tg = st.slider("Terminal (%)", 0, 4, 1, 1) / 100
with base_col:
...
with bull:
    st.markdown("### 🐂 Bull")
    bull_g = st.slider("Growth (%) ", -10, 32, 4, 1) / 100
    bull_wacc = st.slider("WACC (%) ", 5, 20, 6, 1) / 100
    bull_tg = st.slider("Terminal (%) ", 0, 4, 3, 1) / 100
